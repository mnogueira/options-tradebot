"""Bear call spread candidate generation."""
...
def build_bear_call_candidates(
    *,
    venue: str,
    ...
    for short_snapshot in calls:
        ...
        for long_snapshot in calls:
            ...
            if not is_long_leg_tradeable(long_snapshot, liquidity, min_dte=min_dte, max_dte=max_dte):
                continue
            width = long_snapshot.contract.strike - short_snapshot.contract.strike
            ...
            def payoff(terminal_prices):
                pnl = entry_credit - (terminal_prices - short_snapshot.contract.strike).clip(min=0.0)
                pnl += (terminal_prices - long_snapshot.contract.strike).clip(min=0.0)
            ...
            candidates.append(
                StrategyTradeCandidate(
                    candidate_id=(
                        f"bear_call:{venue}:{short_snapshot.contract.underlying}:"
                    ...
                    breakeven_low=None,
                    breakeven_high=short_snapshot.contract.strike + entry_credit,
                    ...
                    score=0.0,
                    thesis="short upside wing richness with capped tail risk",
                )
            )
    return candidates

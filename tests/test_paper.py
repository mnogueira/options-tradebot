...
class PaperBrokerTests(unittest.TestCase):
    ...
    def test_short_option_position_books_credit_and_profit_on_cover(self) -> None:
        today = date(2026, 3, 24)
        snapshot = OptionSnapshot(
            contract=OptionContract(
                symbol="PETR4C1",
            ...
            quote=OptionQuote(bid=1.95, ask=2.05, last=2.00, volume=3000, open_interest=9000),
            timestamp=today,
            underlying_price=47.3,
            risk_free_rate=0.14,
            implied_vol=0.30,
        )
        broker = PaperBroker(initial_cash=5_000.0)
        signal = StrategySignal(
            ...
            stop_price=2.35,
            fair_value=1.50,
            fair_volatility=0.26,
            reason="scanner_short_vol",
            score=2.0,
        )
        self.assertTrue(broker.open_position(signal, snapshot))
        self.assertAlmostEqual(broker.cash, 5_390.0)
        self.assertLess(broker.equity(), broker.cash)
        cover_snapshot = OptionSnapshot(
            contract=snapshot.contract,
            quote=OptionQuote(bid=1.32, ask=1.38, last=1.35, volume=2800, open_interest=8800),
            timestamp=today,
            underlying_price=47.0,
            risk_free_rate=0.14,
            implied_vol=0.25,
        )
        trade = broker.close_position(cover_snapshot, "target")
        self.assertGreater(trade.pnl, 0.0)
        self.assertAlmostEqual(broker.cash, 5_114.0)
